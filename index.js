...
function showToast(message, type) {
    ...
    toast.innerHTML =
        ...
        label +
        '</span><button type="button" class="toast-close" aria-label="Zamknij">&times;</button></div><div class="toast-message">' +
        (message || "").replace(/</g, "&lt;") +
        "</div>";
...
const ARCHIVE_JSON_URL =
    "https://raw.githubusercontent.com/s-pro-v/json-lista/refs/heads/main/linki/linki.json";
const archiveLinksEl = document.getElementById("archive-links");

// Active_Node_Map: fetch home.json
const HOME_JSON_URL =
    "https://raw.githubusercontent.com/s-pro-v/json-lista/refs/heads/main/linki/home.json";
...
function fetchAuthToken() {
    return fetch(AUTH_JSON_URL + "?t=" + Date.now())
        .then(function (res) {
            return res.ok
                ? res.json()
                : Promise.reject(new Error("auth.json nie dostępny"));
        })
        .then(function (data) {
            // 1. Zabezpieczenie: jeśli to tablica, weź pierwszy element. Jeśli obiekt, weź go w całości.
            const entry = Array.isArray(data) ? data[0] : data;

            // 2. Weryfikacja czy w ogóle mamy poprawny obiekt JSON
            if (!entry || typeof entry !== "object") {
                return Promise.reject(new Error("auth.json: nieprawidłowy format"));
            }

            // 3. Sprawdzenie, czy klucz matrix_pat (lub ewentualnie sys_pat) istnieje
            const patToDecode = entry.matrix_pat || entry.sys_pat;

            if (!patToDecode) {
                return Promise.reject(new Error("auth.json: brak matrix_pat"));
            }

            // 4. Dekodowanie
            var token = xorDecrypt(patToDecode, AUTH_XOR_KEY);
            return (
                token ||
                Promise.reject(
                    new Error("Odszyfrowanie nie powiodło się (sprawdź AUTH_XOR_KEY)"),
                )
            );
        });
}
        ...
        .then((res) => {
            if (!res.ok)
                return res
                    .json()
                    .then((j) =>
                        Promise.reject(new Error(j.message || "HTTP " + res.status)),
                    );
            return res.json();
        })
        .then((file) => ({
...
function putJsonListaFile(path, content, sha, message) {
    ...
    const url = GITHUB_API + "/repos/" + JSON_LISTA_REPO + "/contents/" + path;
    const body = {
        message: message || "Hub: aktualizacja " + path,
...
function loadArchiveLinks() {
    ...
    archiveLinksEl.innerHTML =
        '<p class="empty-state-msg">LOADING_REGISTRY...</p>';
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            if (nodes.length === 0) {
                archiveLinksEl.innerHTML =
                    '<p class="empty-state-msg">ARCHIVE_EMPTY. NO_NODES. Kliknij „Dodaj link”.</p>';
                updateLinksCounts();
                return;
            }
            archiveLinksEl.innerHTML = nodes
                .map((node) => {
        ...
        .catch((err) => {
            archiveLinksEl.innerHTML =
                '<p class="empty-state-msg--error">ARCHIVE_OFFLINE. ACCESS_DENIED.</p>';
...
function loadNetworkLinks() {
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            if (nodes.length === 0) {
                networkLinksEl.innerHTML =
                    '<p class="empty-state-msg">NO_ACTIVE_NODES. Kliknij „Dodaj link”.</p>';
                updateLinksCounts();
                return;
            }
            networkLinksEl.innerHTML = nodes
                .map((node) => {
                    const id = (node.id || "").replace(/"/g, "&quot;");
                    const name = (node.name || node.id || "").replace(/"/g, "&quot;");
                    const url = (node.url || "#").replace(/"/g, "&quot;");
        ...
        .catch((err) => {
            networkLinksEl.innerHTML =
                '<p class="empty-state-msg--error">NODE_MAP_OFFLINE. ACCESS_DENIED.</p>';
    ...
    fetchJsonListaFile(JSON_LISTA_HOME_PATH)
        .then(({ content, sha }) => {
            ...
            const next = arr.filter(function (n) {
                return String(n.id || "").trim() !== idStr;
            });
            if (next.length === arr.length) {
                return Promise.reject(new Error("Nie znaleziono linku o id: " + idStr));
            }
            return putJsonListaFile(
                JSON_LISTA_HOME_PATH,
                JSON.stringify(next, null, 4),
                sha,
                "Hub: usunięto link " + idStr,
            );
        })
        .then(function () {
            loadNetworkLinks();
            logToTerminal("Network: link usunięty z GitHub");
            showToast("Link usunięty z GitHub.", "success");
        })
        .catch(function (err) {
            logToTerminal("Network: błąd usuwania");
            showToast(err.message || "Błąd usuwania", "critical");
        });
}
...
function addNetworkLink(payload) {
    ...
    fetchJsonListaFile(JSON_LISTA_HOME_PATH)
        ...
        .then(() => {
            loadNetworkLinks();
            logToTerminal("Network: link dodany");
            showToast("Link dodany.", "success");
        })
        .catch((err) => {
            logToTerminal("Network: błąd dodawania");
            showToast(err.message || "Błąd dodawania", "critical");
        });
}
...
function doDeleteArchiveLink(id) {
    ...
    fetchJsonListaFile(JSON_LISTA_LINKI_PATH)
        ...
        .then(function () {
            loadArchiveLinks();
            logToTerminal("Archive: link usunięty z GitHub");
            showToast("Link usunięty z GitHub.", "success");
        })
        .catch(function (err) {
            logToTerminal("Archive: błąd usuwania");
            showToast(err.message || "Błąd usuwania", "critical");
        });
}
...
function slugId(name) {
    return (
        (name || "")
            ...
            .toLowerCase()
            .replace(/\s+/g, "-")
            .replace(/[^a-z0-9-]/g, "") || "link-" + Date.now()
    );
}
    ...
    fetchJsonListaFile(JSON_LISTA_LINKI_PATH)
        .then(({ content, sha }) => {
            ...
            if (data.nodes.some((n) => (n.id || "") === id)) {
                throw new Error(
                    "Link o takiej nazwie już istnieje (id: " + id + "). Zmień nazwę.",
                );
            }
        ...
        .then(() => {
            loadArchiveLinks();
            logToTerminal("Archive: link dodany");
            showToast("Link dodany.", "success");
        })
        .catch((err) => {
            logToTerminal("Archive: błąd dodawania");
            showToast(err.message || "Błąd dodawania", "critical");
        });
}
...
function syncDeleteToGitHub(noteId) {
    ...
    if (!token) return;
    const getUrl =
        GITHUB_API + "/repos/" + GITHUB_REPO + "/contents/" + GITHUB_SNOTE_PATH;
    fetch(getUrl, {
        headers: {
            Accept: "application/vnd.github+json",
            Authorization: "Bearer " + token,
        ...
        .then((file) => {
            ...
            const putUrl =
                GITHUB_API + "/repos/" + GITHUB_REPO + "/contents/" + GITHUB_SNOTE_PATH;
            const putBody = {
                message: "Hub: usunięto notatkę " + noteId,
        ...
        .catch((err) => {
            setGitHubStatus(
                "Błąd usuwania z GitHub: " + (err.message || err),
                "error",
            );
...
const confirmDeleteOverlayEl = document.getElementById(
...
document
    .getElementById("confirm-delete-cancel-btn")
    ?.addEventListener("click", closeConfirmDeleteModal);
...
confirmDeleteOverlayEl?.addEventListener("click", (e) => {
    if (e.target === confirmDeleteOverlayEl) closeConfirmDeleteModal();
});

let pendingLinkDelete = null;
const confirmDeleteLinkOverlayEl = document.getElementById(
    "confirm-delete-link-overlay",
);
const confirmDeleteLinkModalEl = document.getElementById(
    "confirm-delete-link-modal",
);
const confirmDeleteLinkTitleEl = document.getElementById(
    "confirm-delete-link-title",
);
const confirmDeleteLinkTextEl = document.getElementById(
    "confirm-delete-link-text",
);

const confirmDeleteLinkOkBtn = document.getElementById(
    "confirm-delete-link-ok",
);
...
function openConfirmDeleteLinkModal(id, displayName, listType) {
    ...
    if (confirmDeleteLinkTitleEl)
        confirmDeleteLinkTitleEl.textContent = "Usunąć link?";
    if (confirmDeleteLinkTextEl)
        confirmDeleteLinkTextEl.textContent =
            "„" +
            label +
            "” zostanie usunięty z GitHub" +
            (listType === "network" ? " (Active_Node_Map)." : " (archiwum).");
    ...
    if (confirmDeleteLinkModalEl)
        confirmDeleteLinkModalEl.classList.add("active");
    if (typeof lucide !== "undefined") lucide.createIcons();
}
...
function closeConfirmDeleteLinkModal() {
    ...
    if (confirmDeleteLinkModalEl)
        confirmDeleteLinkModalEl.classList.remove("active");
}

document
    .getElementById("confirm-delete-link-cancel")
    ?.addEventListener("click", closeConfirmDeleteLinkModal);
document
    .getElementById("confirm-delete-link-cancel-btn")
    ?.addEventListener("click", closeConfirmDeleteLinkModal);
...
confirmDeleteLinkOverlayEl?.addEventListener("click", (e) => {
    if (e.target === confirmDeleteLinkOverlayEl) closeConfirmDeleteLinkModal();
});
...
function setGitHubStatus(text, state) {
    ...
    if (githubStatusEl) {
        githubStatusEl.textContent =
            state === "ok" ? "OK" : state === "error" ? text || "Błąd" : text || "—";
        githubStatusEl.className =
            "resource-val" +
            (state === "ok"
                ? " status-ok"
                : state === "error"
    ...
    fetch(getUrl, {
        ...
        .then((file) => {
            ...
            notes.forEach((n) => {
                const entry = {
                    id: n.id,
                    data: n.updatedAt || n.createdAt || new Date().toISOString(),
                    note: JSON.stringify({
                        title: n.title || "",
                        content: n.content || "",
                    }),
            ...
            const notes = list.map((e) => {
                ...
                if (e.note) {
                    try {
                        ...
                        if (parsed && typeof parsed.title !== "undefined")
...
document
    .getElementById("note-github-token-btn")
    ?.addEventListener("click", openGitHubTokenModal);
document
    .getElementById("note-push-github-btn")
    ?.addEventListener("click", pushNotesToGitHub);
document
    .getElementById("note-pull-github-btn")
    ?.addEventListener("click", pullNotesFromGitHub);
document
    .getElementById("github-token-cancel")
    ?.addEventListener("click", closeGitHubTokenModal);
document
    .getElementById("github-token-load-auth-btn")
...
githubTokenModalEl?.addEventListener("click", (e) => {
    if (e.target === githubTokenModalEl) closeGitHubTokenModal();
});
...
function openLinkAddModal(listType) {
    ...
    linkAddTitleEl.textContent =
        listType === "network"
            ? "Dodaj link — Active_Node_Map"
            : "Dodaj link — Archive";
...
const linksEditModeBtnArchive = document.getElementById(
    "links-edit-mode-btn-archive",
);
...
function setLinksEditMode(on) {
    ...
    if (linksEditModeBtn)
        linksEditModeBtn.textContent = linksEditMode
            ? "Wyłącz edycję"
            : "Tryb edycji";
    if (linksEditModeBtnArchive)
        linksEditModeBtnArchive.textContent = linksEditMode
            ? "Wyłącz edycję"
            : "Tryb edycji";
}
...
document
    .getElementById("network-add-link-btn")
    ?.addEventListener("click", () => openLinkAddModal("network"));
document
    .getElementById("archive-add-link-btn")
    ?.addEventListener("click", () => openLinkAddModal("archive"));
document
    .getElementById("link-add-cancel")
    ?.addEventListener("click", closeLinkAddModal);
...
linkAddModalEl?.addEventListener("click", (e) => {
    if (e.target === linkAddModalEl) closeLinkAddModal();
});

document
    .getElementById("network-github-token-btn")
    ?.addEventListener("click", openGitHubTokenModal);
document
    .getElementById("archive-github-token-btn")
    ?.addEventListener("click", openGitHubTokenModal);
...
function renderNotesList() {
    ...
    if (notes.length === 0) {
        notesListEl.innerHTML =
            '<p class="empty-state-msg">Brak notatek. Kliknij „Nowa notatka”, aby dodać.</p>';
        return;
    }
    notesListEl.innerHTML = notes
        .map((note) => {
            ...
            const meta = date
                ? `<span class="note-card__meta">${date.slice(0, 10)}</span>`
                : "";
            return (
                '<div class="note-card ripple-el" data-id="' +
                note.id.replace(/"/g, "&quot;") +
                '">' +
    ...
    notesListEl.querySelectorAll(".note-card").forEach((card) => {
        ...
        card.querySelector(".note-card__delete")?.addEventListener("click", (e) => {
            e.stopPropagation();
...
document.getElementById("note-save-btn")?.addEventListener("click", () => {
    ...
    const note = {
        ...
        content: noteInputContent.value,
    };
    saveNote(note);
    closeNoteModal();
});
...
function createRipple(e) {
    ...
    const x =
        (e.clientX != null ? e.clientX : e.touches && e.touches[0].clientX) -
        rect.left;
    const y =
        (e.clientY != null ? e.clientY : e.touches && e.touches[0].clientY) -
        rect.top;
